...
"""
    ...
    def download_with_progress(self, url, local_path):
        """Download file with a simple progress bar using requests"""
        try:
            ...
            print(f"\n{self.colors.RED}[!] Download failed: {e}{self.colors.ENDC}")
            return False
    ...
    def download_tool(self, tool_name, url, target_name=None, archive=False):
        ...
        if shutil.which(tool_name):
            return True
        print(f"{self.colors.YELLOW}[*] {tool_name} not found. Downloading...{self.colors.ENDC}")
        try:
            tool_dir = os.path.join(os.getcwd(), "tools")
            os.makedirs(tool_dir, exist_ok=True)
            target = os.path.join(tool_dir, target_name or tool_name)
            
            if not self.download_with_progress(url, target):
                return False
            ...
            if os.geteuid() == 0:
                ...
                print(f"{self.colors.GREEN}[+] {tool_name} installed to /usr/local/bin/{self.colors.ENDC}")
            else:
                print(f"{self.colors.GREEN}[+] {tool_name} downloaded to {target}{self.colors.ENDC}")
                print(f"{self.colors.YELLOW}[!] Not root. You may need to add {tool_dir} to your PATH or use the full path.{self.colors.ENDC}")
            return True
        except Exception as e:
    ...
    def setup_serveo(self, lport):
        print(f"{self.colors.CYAN}[SERVEO]{self.colors.ENDC} Requires SSH (usually preinstalled).")
    ...
    def setup_localxpose(self, lport):
        if not self.download_tool("localxpose", "https://api.localxpose.io/api/v2/downloads/loclx-linux-amd64.zip", "loclx.zip", archive=True):
            return None, None
