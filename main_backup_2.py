...
"""
    ...
    def setup_localxpose(self, lport):
        ...
        if token:
            subprocess.run([shutil.which("loclx") or "./tools/loclx", "account", "login", "--token", token], capture_output=True)
        proc = subprocess.Popen(
            [shutil.which("loclx") or "./tools/loclx", "tunnel", "tcp", "--to", f"localhost:{lport}"],
    ...
    def setup_pinggy(self, lport):
        """Pinggy tunnel setup – with token handling and free/pro options"""
        ...
        if not self.pinggy_token:
            ...
            if choice == 'y':
                token = input(f"{self.colors.YELLOW}[?] Enter your Pinggy Pro token: {self.colors.ENDC}").strip()
                if token:
        ...
        if self.pinggy_token:
            ...
            ssh_cmd = [
                ...
                f"{self.pinggy_token}+force+tcp@pro.pinggy.io"
            ]
            public_host = self.pinggy_domain if self.pinggy_domain else f"{self.pinggy_token[:8]}.pinggy.link"
        else:
            # Free mode
            ...
            if subdomain:
                ...
                public_host = f"{subdomain}.a.pinggy.link"
            else:
                ssh_cmd = [
                    "ssh", "-p", "443",
                    "-R", f"0:localhost:{lport}",
        ...
        masked = " ".join(ssh_cmd[:6]) + " ****@..." if self.pinggy_token else " ".join(ssh_cmd)
    ...
    # ===== HTTP TUNNEL SETUP FOR GLOBAL FILE SHARING =====
    def setup_http_tunnel(self, local_port, tunnel_type):
        """Set up an HTTP tunnel for the local HTTP server and return the public URL."""
        if tunnel_type == "ngrok":
            ...
            self.ngrok_process = subprocess.Popen(
                [shutil.which("ngrok") or "./tools/ngrok", "http", str(local_port)],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            print(f"{self.colors.GREEN}[*] Ngrok starting on port {local_port}...{self.colors.ENDC}")
            time.sleep(3)
            try:
                resp = requests.get("http://localhost:4040/api/tunnels", timeout=5)
                tunnels = resp.json().get("tunnels", [])
                for t in tunnels:
                    if t["proto"] == "https" or t["proto"] == "http":
                        return t["public_url"]
            except:
                pass
            print(f"{self.colors.YELLOW}[*] Could not get ngrok URL automatically. Check dashboard.{self.colors.ENDC}")
            return None
            ...
            if token:
                subprocess.run([shutil.which("loclx") or "./tools/loclx", "account", "login", "--token", token], capture_output=True)
            # Start loclx HTTP tunnel
            proc = subprocess.Popen(
                [shutil.which("loclx") or "./tools/loclx", "tunnel", "http", "--to", f"localhost:{local_port}"],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            time.sleep(3)
            print(f"{self.colors.GREEN}[+] LocalXpose tunnel started. Check output for URL.{self.colors.ENDC}")
            url = input(f"{self.colors.YELLOW}[?] Enter the assigned LocalXpose URL (e.g., https://something.loclx.io): {self.colors.ENDC}")
            return url if url else None

        elif tunnel_type == "cloudflared":
            if not self.download_tool("cloudflared", "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64", "cloudflared"):
                return None
            # Cloudflare can run without token for quick tunnels (trycloudflare.com)
            print(f"{self.colors.YELLOW}[*] Starting cloudflared tunnel (no token needed for temporary URL)...{self.colors.ENDC}")
            proc = subprocess.Popen(
                [shutil.which("cloudflared") or "./tools/cloudflared", "tunnel", "--url", f"http://localhost:{local_port}"],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            time.sleep(5)
            print(f"{self.colors.GREEN}[+] Cloudflare tunnel started.{self.colors.ENDC}")
            print(f"{self.colors.YELLOW}[*] Look for a line like: https://random-name.trycloudflare.com{self.colors.ENDC}")
            url = input(f"{self.colors.YELLOW}[?] Enter the Cloudflare tunnel URL: {self.colors.ENDC}")
            return url if url else None

        elif tunnel_type == "serveo":
            print(f"{self.colors.CYAN}[SERVEO]{self.colors.ENDC} HTTP tunnel via SSH.")
            subdomain = input(f"{self.colors.YELLOW}[?] Enter subdomain (optional): {self.colors.ENDC}")
            remote = f"{subdomain}:80:localhost:{local_port}" if subdomain else f"80:localhost:{local_port}"
            cmd = ["ssh", "-R", remote, "serveo.net", "-o", "StrictHostKeyChecking=no", "-f", "-N"]
            try:
                subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                time.sleep(3)
                url = f"https://{subdomain or local_port}.serveo.net"
                print(f"{self.colors.GREEN}[+] Serveo tunnel started. URL: {url}{self.colors.ENDC}")
                return url
            except Exception as e:
                print(f"{self.colors.RED}[!] Serveo error: {e}{self.colors.ENDC}")
                return None
            ...
            # Pinggy HTTP tunnel
            print(f"{self.colors.CYAN}[PINGGY]{self.colors.ENDC} HTTP tunnel via SSH.")
            if not self.pinggy_token:
                print("Pinggy free mode (random URL) or Pro?")
                choice = input(f"{self.colors.YELLOW}[?] Use Pinggy Pro? (y/n, default n): {self.colors.ENDC}").lower()
                if choice == 'y':
                    token = input(f"{self.colors.YELLOW}[?] Enter your Pinggy Pro token: {self.colors.ENDC}").strip()
                    if token:
                        ...
                        print(f"{self.colors.YELLOW}[!] No token, using free mode.{self.colors.ENDC}")
            if self.pinggy_token:
                ssh_cmd = [
                    "ssh", "-p", "443",
                    ...
                    "-o", "StrictHostKeyChecking=no",
                    "-o", "ServerAliveInterval=30",
                    f"{self.pinggy_token}@pro.pinggy.io"
                ]
                public_host = self.pinggy_domain if self.pinggy_domain else f"{self.pinggy_token[:8]}.pinggy.link"
                ...
                if subdomain:
                    ...
                    url = f"https://{subdomain}.a.pinggy.link"
                else:
                    ssh_cmd = [
                        "ssh", "-p", "443",
                        "-R", f"0:localhost:{local_port}",
                    ...
                    url = "https://live.pinggy.io (check output for exact URL)"
            ssh_cmd.insert(1, "-f")
            try:
                subprocess.Popen(ssh_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                time.sleep(3)
                print(f"{self.colors.GREEN}[+] Pinggy tunnel started.{self.colors.ENDC}")
                if "live.pinggy.io" in url:
                    # In free mode without subdomain, the URL is printed; we need user to input
                    url_input = input(f"{self.colors.YELLOW}[?] Enter the Pinggy URL shown (e.g., https://xxxx.pinggy.link): {self.colors.ENDC}")
                    url = url_input
                else:
                    print(f"{self.colors.GREEN}[+] Public URL: {url}{self.colors.ENDC}")
                return url
            except Exception as e:
                print(f"{self.colors.RED}[!] Pinggy error: {e}{self.colors.ENDC}")
                return None
...
'''
    ...
    def start_http_server(self, port=8080, global_access=False):
        if self.http_server:
